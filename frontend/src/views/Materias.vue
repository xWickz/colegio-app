...
import { getSubjectsLength } from '@/services/queryLengths'
import { fetchMaterias } from '@/services/materiasService'
import { addMateriaService } from '@/services/materiasService'
...
const totalPages = computed(() => Math.ceil(subjectsLength.value / limit.value))

// Queries
const fetchData = async () => {
    try {
        items.value = await fetchMaterias(page.value, limit.value, search.value) // pagina, limites
...
const addMateria = async () => {
    if (!nombre.value) return alert('Llena ambos campos')

    try {
        await addMateriaService(nombre.value)
        nombre.value = ''
        await fetchData()
...
const openInfoModal = async (subject) => {
...
<template>
    ...
    <main class="bg-gray-100 min-h-screen px-20 py-10 ml-60 mx-auto dark:bg-dark">
        ...
        <div id="toast-success"
            ...
            <div class="ms-3 text-sm font-normal">Materia añadida correctamente</div>
            <button type="button"
                ...
                <div class="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
                    ...
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                        <div class="col-span-2">
                            <label for="name" class="block mb-2.5 text-sm font-medium text-heading">Nombre de la Materia</label>
                    ...
                    <div class="flex items-center space-x-4 border-t border-default pt-4 md:pt-6">
                        <button @click="addMateria" type="submit"
        ...
        <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
            <div class="p-4">
                <label for="input-group-1" class="sr-only">Busqueda de Materia</label>
                <div class="relative">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-body">
                ...
                <tbody>
                    <tr v-for="materia in items" :key="materia.id"
                        ...
                        <td class="px-6 py-4">
                            <div class="relative">
                                <button :id="'accionesButton_' + materia.id" @click="toggleDropdown(materia.id)"
                                ...
                                <div v-if="openDropdownId === materia.id"
                                    ...
                                    <ul class="p-2 text-sm text-body font-small"
                                        :aria-labelledby="'accionesButton_' + materia.id">
                                        <li>
                                            <a href="#"
                                        ...
                                        <li>
                                            <button type="button"
                                                ...
                                                @click="openInfoModal(materia)">Ver Info.</button>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center justify-between border-b border-default pb-4 md:pb-5">
                                        <h2 class="text-2xl font-bold text-heading">
                                            {{ selectedSubject.materia_nombre }}
                                        </h2>
                                        <button type="button"
                                            class="text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center"
                                    ...
                                    <div class="space-y-4 md:space-y-6 py-4 md:py-6">
                                        <p class="leading-relaxed text-body">
                                            <strong>Nombre:</strong> {{ selectedSubject.materia_nombre }}
                                        </p>
                                    </div>
            ...
            <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between p-4"
                ...
                <span class="text-sm font-normal text-body mb-4 md:mb-0 block w-full md:inline md:w-auto">Mostrando
                    <span class="font-semibold text-heading">{{ (page - 1) * limit + 1 }}-{{ Math.min(page * limit,
                        subjectsLength) }}</span> de <span class="font-semibold text-heading">{{ subjectsLength
                        }}</span></span>
